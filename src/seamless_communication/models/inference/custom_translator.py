...
from seamless_communication.models.encoder import encoder_inference
from seamless_communication.models.custom_vocoder.vocoder import Vocoder, init_vocoder
...
class Translator(nn.Module):
    def __init__(
        ...
        self.vocoder: Vocoder = init_vocoder("src/seamless_communication/models/custom_vocoder/model_config.json").to(device)
        state_dict_g = load_checkpoint(vocoder_path, device)
        self.vocoder.code_generator.load_state_dict(state_dict_g['generator'])
        self.vocoder.eval()
        self.vocoder.code_generator.eval()
        
        if not encoder_inference.is_loaded():
    ...
    def predict(
        ...
        if input_modality == Modality.SPEECH:
            ...
            if isinstance(audio, str):
                wav = encoder_inference.preprocess_wav(audio)
                embed_spkr = encoder_inference.embed_utterance(wav)
                print(len(embed_spkr))
                print(embed_spkr)
        ...
        if output_modality == Modality.TEXT:
            ...
            print(units) 
            print(tgt_lang)
            wav_out = self.vocoder(units, tgt_lang, embed_spkr, dur_prediction=False)
